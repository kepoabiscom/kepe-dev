...
class Comment_model extends CI_Model {
    ...
    function get_comment($type='', $id='', $f=0) {
    	...
    	$query = $this->db->select($type."_comment_id, nick_name, img, ip_address, body, created_date")
    			->from($type . "_comment")
    			->where($type."_id", $id)
    ...
    function get_list_comment($type='') {
        $query = $this->db->select($type."_comment_id as id, nick_name, img, ip_address, body, created_date")
                ->from($type . "_comment")
                ->get();
